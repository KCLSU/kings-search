...
const varsityTotalScoreCss = globalstylesCss + "div{height:100%;width:100%;position:relative;background:linear-gradient(to right, #6c3f99 0%,#6c3f99 51%,#6c3f99 51%,#e2231a 52%,#e2231a 100%)}span{padding:0em;margin:0em;font-size:25px;text-align:center}.ucl,.kings{padding:1em;margin:0.5em;font-size:45px;font-weight:bold;color:white}.logo{position:absolute;left:50%;top:10%;transform:translate(-50%, -50%);width:8em;height:auto}img{display:block;width:100%;height:auto}@media only screen and (max-width: 780px){.ucl,.kings{font-size:35px}}@media only screen and (max-width: 500px){.ucl,.kings{padding:0.5em;line-height:40px}.logo{width:7em}}";

const VarsityTotalScore = class {
    constructor(hostRef) {
    ...
    calculateScores() {
        ...
        this.scores.forEach(ob => {
            if (+ob.score[0] > +ob.score[1])
                this.kings++;
            else if (+ob.score[0] < +ob.score[1])
                this.ucl++;
            else if (+ob.score[1] === +ob.score[0]) {
                this.ucl++;
                this.kings++;
            }
        });
    ...
    render() {
        ...
        return (h("div", null, h("flex-container", { alignx: "center", aligny: "center" }, h("span", { class: "ucl" }, "UCL  ", this.ucl), h("div", { class: "logo" }, h("img", { src: "https://kcl-dev.ukmsl.net/pageassets/varsity/VarsityV.png" })), h("span", { class: "kings" }, "Kings ", this.kings))));
    }
};
VarsityTotalScore.style = varsityTotalScoreCss;

export { VarsityTotalScore as varsity_total_score };

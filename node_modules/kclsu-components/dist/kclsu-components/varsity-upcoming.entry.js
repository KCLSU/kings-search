...
import { g as getNextEvents, r as returnDate } from './utils-cccb6bd3.js';
...
const varsityUpcomingCss = globalstylesCss + "ul{margin:0;margin-top:3.5em;padding:0}li{list-style:none;padding:0em;color:black;cursor:pointer;-webkit-box-shadow:0.5px 3px 5px 0px rgba(0,0,0,0.15);-moz-box-shadow:.5px 3px 5px 0px rgba(0,0,0,0.15);box-shadow:0.5px 3px 5px 0px rgba(0,0,0,0.15);font-size:16px;margin:1em;padding:0.6em;position:relative}a{margin:0;padding:0}.button{display:table;padding:5px 20px;margin:15px 20px 10px;margin:auto;color:white;background:#14847e;border-radius:30px;font-size:18px;cursor:pointer;-webkit-box-shadow:4px 3px 9px -2px rgba(0, 0, 0, .3);-moz-box-shadow:4px 3px 9px -2px rgba(0, 0, 0, .3);box-shadow:4px 3px 9px -2px rgba(0, 0, 0, .3)}.date{position:absolute;font-size:12px;bottom:0;right:0;color:#6c3e99}";

const VarsityUpcoming = class {
    constructor(hostRef) {
    ...
    mapToLi(data) {
        ...
        if (!sliced)
            return h("a", { href: '/events' }, h("li", null, "0 Varsity Events Remaining"));
        else
            return sliced.map(node => {
                let date = returnDate(node.StartDate);
                return h("a", { href: node.Url }, h("li", { "data-id": node.Id }, node.Title, h("span", { class: "date" }, date.weekday, " ", date.day, "th")));
            });
    }
    closeModal() {
        this.showall = false;
    ...
    render() {
        return ([
            h("kclsu-modal", { show: this.showall }, !this.showall ? '' : h("varsity-next-matches", { data: getNextEvents(this.data) }, " ")),
            h("ul", null, this.mapToLi(this.data), h("a", { onClick: (e) => this.clickHandler(e), class: "button" }, "See All Upcoming "))
        ]);
    }
};
VarsityUpcoming.style = varsityUpcomingCss;

export { VarsityUpcoming as varsity_upcoming };

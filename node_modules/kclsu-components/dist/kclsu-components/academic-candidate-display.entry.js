...
const academicCandidateDisplayCss = globalstylesCss + ".academic_sub{color:#502669;padding:0 10px;font-size:14px;box-sizing:border-box;padding:0.5em;list-style:none;margin:0}.academic_sub:hover{cursor:pointer}.academic_sub::after{content:'';border-right:1px solid #502669;margin:0.5em;margin-left:1.5em}.positions{padding:0;margin:0;margin-bottom:2em}.submenu{display:flex;justify-content:center;flex-flow:row wrap;margin:0;padding:0}@media only screen and (min-width: 1000px){section{margin-top:4em}}@media only screen and (max-width: 1000px){.academic_sub{border:none;font-size:14px;margin:1em;padding:0;line-height:normal}.academic_sub::after{margin:1em}.submenu{justify-content:flex-start}}@media only screen and (max-width: 500px){.academic_sub{margin:10px 0;font-size:13px}.academic_sub::after{border:none}}";

const AcademicCandidateDisplay = class {
    constructor(hostRef) {
        ...
        if (this.data) {
            for (let x = 0; x < this.data.length; x++) {
                ...
                if (!positions.find(p => p === current.Post)) {
                    let link = h("li", { role: "tab", id: current.Post, "data-candidates": current, class: "academic_sub", "aria-selected": "false", onClick: e => this.clickHandler(e) }, current.Post);
    ...
    render() {
        let candidates = !this.current ? '' : h("candidate-display", { data: this.data.filter(candidate => candidate.Post === this.current) });
        return (h("div", { role: "presentation" }, h("div", { class: "positions", role: "presentation" }, h("ul", { class: "submenu", role: "tablist" }, this.createSubLinks())), h("section", { role: "tabpanel", "aria-labelledby": this.current }, candidates)));
    }
    get host() { return getElement(this); }
};
AcademicCandidateDisplay.style = academicCandidateDisplayCss;

export { AcademicCandidateDisplay as academic_candidate_display };

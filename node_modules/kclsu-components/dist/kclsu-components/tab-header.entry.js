...
const tabHeaderCss = globalstylesCss + ".tab-header{min-height:20px;display:flex;justify-content:center;flex-wrap:wrap;align-items:center;text-transform:uppercase;padding:12px;color:#502766;background:#e8e8e2;font-weight:500;list-style:none;box-sizing:content-box;border-bottom:3px solid transparent;}.tab-header a{cursor:pointer;font-family:bree_serifbold, bree-serif, serif;margin:0 0 0px;font-weight:700;color:#502766;background:#e8e8e2}@media (max-width: 599px){.tab-header{min-width:100px;margin:0;padding:5px}}.tab-header-active{border-bottom:3px solid #1BA39C}";
...
const TabHeader = class {
    constructor(hostRef) {
        ...
        this.selectTabByIndex = createEvent(this, "selectTabByIndex", 7);
    }
    componentDidLoad() {
        if (this.active) {
    ...
    render() {
        ...
        if (this.active) {
            return (h("li", { role: "presentation", class: classes }, h("a", { href: "#", onClick: (e) => this.onClick(e), id: this.name, "aria-selected": this.active, role: "tab" }, h("slot", null))));
        }
        else {
            return (h("li", { role: "presentation", class: classes }, h("a", { href: "#", onClick: (e) => this.onClick(e), id: this.name, role: "tab" }, h("slot", null))));
        }
    }

...
const candidatesDisplayCss = globalstylesCss + ".card-layout{display:grid;grid-gap:30px;grid-template-columns:repeat(auto-fit, minmax(260px, 1fr));justify-items:center}@media only screen and (max-width: 810px){.card-layout{grid-gap:20px;grid-template-columns:repeat(auto-fit, minmax(210px, 1fr))}}";

const CandidatesDisplay = class {
    constructor(hostRef) {
        registerInstance(this, hostRef);
    }
    render() {
        const cards = !this.data ? null : this.data.map(candidate => {
            return h("profile-card", { name: candidate.Name.toLowerCase(), position: candidate.Post, link: candidate.ManifestoLink, image: candidate.ImageLink ? candidate.ImageLink : 'https://res.cloudinary.com/kclsu-media/image/upload/f_auto,fl_any_format,g_center,q_100/v1581516201/website_uploads/KCLSU%20Brand/Bzcl1r6L_400x400_se7grm.jpg', cta: 'Manifesto', secondcta: 'Breakdown', secondlink: candidate.ResultsLink });
        });
        return (h("profile-card-layout", null, cards));
    }
};
CandidatesDisplay.style = candidatesDisplayCss;

export { CandidatesDisplay as candidate_display };
